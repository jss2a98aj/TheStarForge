...
function init()
  ...
  self.lastProjectilePosition = mcontroller.position()
...
function update(dt)
  if self.hostProjectile and world.entityExists(self.hostProjectile) then
    local pos = world.entityPosition(self.hostProjectile)
    
    local mag = world.magnitude(world.entityPosition(self.hostProjectile), self.lastProjectilePosition)
    if mag > 0.5 then
      vec2.add(pos, vec2.mul(self.hostVelocity, dt))
    end
  ...
  self.lastProjectilePosition = world.entityPosition(self.hostProjectile)
end
...
function castBoltMultiple(targets, lightning)
  ...
  for i, target in ipairs(targets) do
    if entity.entityInSight(target) and world.entityCanDamage(self.hostEntity, target) and world.entityExists(target) then		
      --Draw lightning
      local bolt = nebUtil.copyTable(lightning or self.defaultBoltConfig, 11)
