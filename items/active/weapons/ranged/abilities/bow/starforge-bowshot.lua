...
-- Bow primary ability
StarforgeBowShot = WeaponAbility:new()

function StarforgeBowShot:init()
...
function StarforgeBowShot:update(dt, fireMode, shiftHeld)
  ...
  if not self.weapon.currentAbility and self.fireMode == (self.activatingFireMode or self.abilitySlot) and self.cooldownTimer == 0 and (self.drawTimer > 0 or not status.resourceLocked(self.resourcetype or "energy")) then
    self:setState(self.draw)
  end
end

function StarforgeBowShot:uninit()
  self:reset()
end

function StarforgeBowShot:reset()
...
function StarforgeBowShot:draw()
  self.weapon:setStance(self.stances.draw)

  animator.playSound("draw", -1)
  local readySoundPlayed = false

  while self.fireMode == (self.activatingFireMode or self.abilitySlot) and not status.resourceLocked(self.resourcetype or "energy") do
	...
	if self.drawTimer < self.drawTime then
	  status.overConsumeResource(self.resourcetype or "energy", self.energyPerShot / self.drawTime * self.dt)
	...
	elseif self.drawTimer > self.drawTime and self.drawTimer <= (self.drawTime + (self.powerProjectileTime or 0)) then
	  ...
	  if self.drainEnergyWhilePowerful then
		  status.overConsumeResource(self.resourcetype or "energy", self.holdEnergyUsage * self.dt) --Optionally drain energy while at max power level
	  end
	...
	elseif self.drawTimer > (self.drawTime + (self.powerProjectileTime or 0)) then
	  status.overConsumeResource(self.resourcetype or "energy", self.holdEnergyUsage * self.dt)
	end
...
function StarforgeBowShot:fire()
  ...
  if not world.lineTileCollision(mcontroller.position(), self:firePosition()) then
    ...
    animator.setGlobalTag("drawFrame", "loosed")
...
function StarforgeBowShot:perfectTiming()
...
function StarforgeBowShot:currentProjectileParameters()
  ...
  --Bonus damage calculation for quiver users from elithian races mod
